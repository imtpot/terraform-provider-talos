...
The `kubernetes_version` argument controls the component image tags in the generated configuration. It does **not** trigger a Kubernetes upgrade on its own. To upgrade Kubernetes, change `talos_cluster.kubernetes_version`, which runs Talos's `upgrade-k8s` procedure.

Keep `kubernetes_version` here in sync with `talos_cluster.kubernetes_version` so nodes added later (scale-up) bootstrap at the correct version. See [`talos_cluster`](../resources/cluster.md) for details.
...
- `talos_version` (String) The Talos version contract used to generate the machine configuration. This does not control the installed Talos version — use `config_patches` to set `machine.install.image` for that. Keep this pinned to the version used when the cluster was created; it is independent of `talos_machine.image`, and regular Talos upgrades do not require bumping it. Only change it when you deliberately want a newer contract's schema and defaults, which regenerates the machine configuration on its own. See [Reproducible Machine Configuration](https://docs.siderolabs.com/talos/latest/configure-your-talos-cluster/system-configuration/reproducible-machine-configuration) for details. Example values: `v1.12`, `v1.12.1`, `1.12`, `1.12.1`
