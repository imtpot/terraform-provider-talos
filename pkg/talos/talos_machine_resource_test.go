...
func TestAccTalosMachine_multidocK8sImagesOwned(t *testing.T) {
	var (
		// talosVersion tracks the current dep so the booted node matches the
		// config documents generated by this version of the machinery package.
		// Hardcoding an older alpha risks "document type not registered" errors
		// when the machinery generates doc kinds unknown to that older node.
		talosVersion = gendata.VersionTag
		baseK8s      = "v1.36.0"
		bumpedK8s    = "v1.36.1"
	)
